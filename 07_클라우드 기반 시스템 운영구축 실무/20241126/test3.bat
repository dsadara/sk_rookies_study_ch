...
@REM :: 10. 파일 생성/수정/접근 이력
@REM echo [10. 최근 파일 변경 이력] >> "%logFile%"
@REM echo 최근 24시간 내 수정된 파일: >> "%logFile%"
@REM forfiles /P C:\ /S /D -1 /C "cmd /c echo @path @fdate @ftime" 2>nul >> "%logFile%"
@REM echo. >> "%logFile%"

@REM :: 11. 시작프로그램 목록
@REM echo [11. 시작프로그램 목록] >> "%logFile%"
@REM echo 시작 메뉴 시작프로그램: >> "%logFile%"
@REM dir /B "%APPDATA%\Microsoft\Windows\Start Menu\Programs\Startup" >> "%logFile%"
@REM echo. >> "%logFile%"
@REM echo 레지스트리 시작프로그램: >> "%logFile%"
@REM reg query "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Run" >> "%logFile%"
@REM reg query "HKEY_CURRENT_USER\SOFTWARE\Microsoft\Windows\CurrentVersion\Run" >> "%logFile%"
@REM echo. >> "%logFile%"

echo [10번] >> "%logFile%"
echo 최근 24시간 내 수정된 파일: >> "%logFile%"
dir /A /O:D /Q /S /T:C C:\ >> "%logFile%"
echo. >> "%logFile%"
